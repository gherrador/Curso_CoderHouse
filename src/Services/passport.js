...
const { FACEBOOK_CLIENT_ID, FACEBOOK_CLIENT_SECRET } = require('../../config/index');
...
const { logger, loggererror, loggerwarn } = require('./logger')
    ...
    passport.use(new FacebookStrategy({
        ...
        findOrCreateUser = function() {
            Usuarios.findOne({ username: userProfile.displayName }, function(err, user) {
                if (err) {
                    loggererror.error('error al registrar:' + err)
                    return done(err)
                }
                if (user) {
