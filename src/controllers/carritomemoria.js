...
const productos = require(path.join(__dirname, './producto'))
...
const guardar = (id) => {
    try {
        ...
        carrito_memoria.push({
            idCarrito: carrito_memoria.length + 1,
            timeStamp: Date.now(),
            producto: {...producto
            }
        });
...
const listarPorId = (id) => {
    try {
        let producto = carrito_memoria.filter((producto) => producto.idCarrito == id)[0]
        return producto
    } catch (err) {
        return []
    }
...
const borrarPorId = (id) => {
    let producto = carrito_memoria.filter((producto) => producto.idCarrito == id)
    let productoABorrar = carrito_memoria.filter((producto) => producto.idCarrito != id)
    if (!producto) {
